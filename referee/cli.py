"""``rethlas referee`` standalone Phase 3 dispatch."""
...
from common.runtime.jobs_v2 import RoleJobRecord, make_role_job_id, write_role_job_file
...
def run_referee(workspace: str | None, args: argparse.Namespace) -> int:
    ws = workspace_paths(workspace)
    ensure_initialised(ws)

    input_packet = _load_json(args.context_json)
    target = args.target or args.source or input_packet.get("target")
    if not isinstance(target, str) or not target:
        sys.stderr.write("referee: --target, --source, or context_json.target is required\n")
        return 2
    input_packet.setdefault("target", target)
    if args.source:
        input_packet.setdefault("source_id", args.source)
    input_packet.setdefault("target_hashes", {})
    input_packet.setdefault("budgets", {})

    if getattr(args, "queue", False):
        item = enqueue_role_item(
            ws.root,
            kind="referee",
            mode="review_source" if args.source else "review_target",
            target=target,
    ...
    job_id = make_role_job_id("referee", iso_ms=iso_ms, uid=uid)
    started = utc_now_iso()
    ...
    from referee.role import main as role_main
