...
def test_saved_moeglin_waldspurger_learner_run_is_reusable(tmp_path: Path) -> None:
    ...
    assert {path.stem for path in artifacts} == set(BATCH_IDS)
    ...
    for artifact in artifacts:
        ...
        assert len(payload["candidate_nodes"]) == BATCH_IDS[artifact.stem]

        total_candidates += len(payload["candidate_nodes"])
        total_verification_requests += len(payload["verification_requests"])
        total_issues += len(payload["issues"])
        labels.update(node["label"] for node in payload["candidate_nodes"])

    assert total_candidates == 22
    assert total_verification_requests == 6
    assert total_issues == 19
